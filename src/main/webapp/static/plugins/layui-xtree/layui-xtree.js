...
layuiXtree.prototype.GetAllChecked = function () {
    ...
    for (var i = 0; i < cks.length; i++) {
		if (cks[i].checked && cks[i].getAttribute('data-xend') == '1') {
            arrAll[arrAllIndex] = cks[i]; arrAllIndex++;
			_this.func(cks[i].value);
        }
    }
    return arrAll;
}
//递归取父节点对象
layuiXtree.prototype.func= function (a){
	var _this = this;
    if (a != null){
		var cks = _this.getByClassName('layui-xtree-checkbox');
		for (var i = 0; i < cks.length; i++) {
			if (cks[i].value == a) {
				...
				var funcValue=_this.getChildByClassName(cks[i].parentNode.parentNode, 'layui-xtree-checkbox')[0];
				var j = arrAll.length;  
				while (j--) {  
					if (arrAll[j] === funcValue) {  
						return arrAll;  
					}  
				}
				arrAll[arrAllIndex] = funcValue;
				arrAllIndex++;
			}
		}
    }
    return _this.func(arrAll[arrAllIndex-1].value);
}
