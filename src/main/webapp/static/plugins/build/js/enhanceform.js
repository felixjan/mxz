...
layui.define([ 'jquery', 'form' ], function(exports) {
	var $ = layui.jquery, form = layui.form, formObj, hint = layui.hint();
	 ...
	 * @returns {}
	 */
	EnhanceForm.prototype.setTextareaVal = function(name, val) {
		if (name === undefined) {
			throw "name no undefined";
		}
		formObj.find('textarea[name="' + name + '"]').val(val);
		return this;
	}
	/**
	 * 设置select选中值
	 ...
	 * @returns {}
	 */
	EnhanceForm.prototype.setSelectVal = function(name, val,htmlVal) {
		if (name === undefined) {
			throw "name no undefined";
		}
		var selectObj=formObj.find('select[name="' + name + '"]');
		var isExist = false;  
	    var count = selectObj.find('option').length;  
	    for(var i=0;i<count;i++){     
         if(selectObj.get(0).options[i].value == val){     
        	 isExist = true;     
             break;     
          }     
        }
	    if(isExist){
	    	...
	    	selectObj.append("<option value="+val+">" + htmlVal + "</option>");
	    	selectObj.val(val);
	    }
	    form.render('select');
		selectObj.find("dd[lay-value='" + val + "']").trigger("click");
		return this;
	};
	/**
	 * 设置radio选中
		...
		for ( var key in data) {
			if (data.hasOwnProperty(key)) {
				...
				if (inputs.length > 0) {
					...
					if (select.length > 0) {
						if(this.endWith(key,'Id')){
							var htmlVal=key.substr(0,key.length-2)+'Name';
							this.setSelectVal(key, data[key],data[htmlVal]);
						}else{
							this.setSelectVal(key, data[key],'');
						}
					}else{
						var textarea = formObj.find('textarea[name="' + key + '"]');
						if (textarea.length > 0) {
							this.setTextareaVal(key, data[key]);
						}
					}
				}
			}
	...
	EnhanceForm.prototype.endWith=function(str,endStr){
	 ...
	 * 接口输出
	 */
	exports('enhanceform', function(options) {
